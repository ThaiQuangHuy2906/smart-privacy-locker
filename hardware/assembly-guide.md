...
1. Complete the acceptance fields in [bom.md](bom.md) and the exact-part worksheet in [power-budget.md](power-budget.md). A product title or seller photo is not an electrical datasheet.
2. Confirm the actual board is compatible with PlatformIO `esp32dev`, identify its USB serial port, and verify its printed GPIO labels.
3. With power disconnected, mark the 5 V, 3.3 V, and ground buses. Verify supply polarity and continuity with a meter.
4. Wire **all grounds first**. Do not connect the SG90 5 V lead to ESP32 3.3 V or a GPIO. Keep every pull-up visible to an ESP32 pin at 3.3 V unless a verified level shifter separates it.
5. Add strain relief and insulation so no wire can touch the enclosure, latch, or stored object. Keep an accessible power-off path.
6. Keep the servo mechanically unloaded for initial angle calibration. Set the candidate angles only after confirming the latch has physical stops and the servo will not stall at either end.
7. Choose one board-approved power arrangement. Do not simultaneously feed external 5 V and an ordinary powered USB cable unless the exact board documentation confirms reverse-current protection.

## Safe incremental bring-up

1. Power the ESP32 alone and confirm serial upload/boot.
2. Add only a 3.3 V-I2C-compatible OLED. With power disconnected, confirm its pull-ups do not connect SDA/SCL to 5 V; then scan the bus and confirm the boot message. The committed default is `AppConfig::OLED_I2C_ADDRESS = 0x3C`. If the scan finds `0x3D`, copy the full example with `Copy-Item firmware\include\app_config.example.h firmware\include\app_config.h`, then change `OLED_I2C_ADDRESS` to `0x3D` directly in that ignored full copy. Do not include the example and redeclare the constant, and do not treat the default as hardware verification.
3. Add the DHT22 at 3.3 V with its data pull-up also terminating at 3.3 V. Keep the initial cable at or below 1 m, wait through several 2.5-second read cycles, and confirm values locally on OLED.
4. Add the verified WS2812 part through the prescribed logic buffer, data resistor, and bulk capacitor. Test `LED_ON`/`LED_OFF` through a broker only after MQTT is configured.
...
10. Run the combined-load worksheet last. A successful single-module test does not prove that servo, LED, buzzer, Wi-Fi and sensors are safe when active together.
