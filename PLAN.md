...
| P1-M09 | MQTT reconnect/LWT | Ngắt Wi-Fi/broker đột ngột rồi khôi phục; nếu broker test có ACL sandbox, từ chối một lần subscribe `command` nhưng vẫn cho connect/publish, rồi khôi phục quyền | LWT `OFFLINE`; backoff; `ONLINE` + full state sau reconnect; subscription fail không để `ONLINE`/full state giả, disconnect và retry bounded | MANUAL — HARD-GATE | Timestamped broker log, ACL/failure configuration nếu dùng |
...
- Implementation: Repository baseline; PlatformIO ESP32 Arduino firmware foundation; WiFiManager; MQTT v1 parser/validation/dedupe/ACK/state/LWT/reconnect; CB2 SG90 state machine with cold-boot no-motion; YC1 local DHT22/OLED error display; YC3 WS2812B control; wiring/power/test/docs created. Targeted patch: a transport connection is usable only after `command` subscription succeeds; a subscription failure keeps MQTT state false, publishes `OFFLINE` when possible, disconnects, and follows bounded retry. OLED I2C address is `AppConfig::OLED_I2C_ADDRESS` (default `0x3C`), not a display-source literal. CB1/YC6/YC8/YC9/CB3/YC4/YC5/YC7 were not implemented.
...
- Commit: `a520372` — `feat(phase-1): add ESP32 firmware foundation`; `3b84bf8` — `docs(phase-1): record active handoff blockers`; `bbc39b0` — `docs(phase-1): record push and PR blocker`; `02a15e2` — `fix(phase-1): harden MQTT subscription recovery`.
