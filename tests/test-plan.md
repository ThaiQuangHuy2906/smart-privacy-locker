...
> Current audit snapshot: automated gates were rerun on 2026-08-18 and pass in
> the scopes below. Individual user observations exist for OLED/DHT22,
> MC-38-to-Telegram, ten WS2812 pixels and SG90 latch travel; they are
...
|---|---|---|---|
| P1-A01 | `.\firmware\build-esp32.ps1` (after an optional clean) | pinned firmware build with validated temporary ASCII drive mapping when required | exit 0; 53,580 RAM bytes; 1,108,145 flash bytes |
| P1-A01B | `.\arduino\verify-sketch.ps1` | Arduino IDE source parity and isolated profile build using Arduino-ESP32 2.0.17 plus pinned libraries | 32 mirrored source files match byte-for-byte; 1,112,269 program bytes and 53,608 RAM bytes |
| P1-A01C | `.\arduino\verify-arduino-ide.ps1` | Arduino IDE global core/library environment and GUI-equivalent build | not rerun after auto-lock; the earlier 1,111,201/53,600-byte result is historical, while current source is covered by P1-A01B |
| P1-A02 | `python -m platformio test -e native` | valid action/UUID/locker/requester parse plus Phase 2/3 firmware regressions | full current native suite passes 28/28 |
...
| P1-A06 | same native suite | local CLOSED→OPEN alarm, bounded FIFO door-event outbox, auto-lock arming/expiry and in-flight latch cancellation | local alarm/auto-lock do not require network; boot does not arm servo; exact expiry is wrap-safe; outbox stays bounded/FIFO; opening during command/automatic travel fails safely |
...
load conditions. They remain useful manual acceptance rows. The unmeasured
power/full-load finding P1-05 was explicitly accepted by the user for the
current demo on 2026-08-18; that waiver is not a measured PASS and must be
reopened for any broader/long-duration deployment.

| ID | Procedure | Expected result | Evidence |
|---|---|---|---|
| P1-M01 | With SG90 unloaded and 5 V rail verified, send alternating valid `UNLOCK`/`LOCK` commands through authenticated broker. | Latch travels to `170°`/`80°` without jitter/reset; ACK/state matches each completed command. | video, angle values, voltage/current note |
| P1-M02 | Mount the SG90 rotating latch to the actual door and repeat `LOCK=80°`/`UNLOCK=170°`. Close/open the door manually and verify MC-38 separately. | Latch moves without stall/excess travel; `LOCK` is blocked/cancelled when the door is open; lack of angle feedback is recorded. | latch photos/video, MC-38 outcome, defect note if any |
...
| P1-M09 | Abruptly stop Wi-Fi or broker, then restore it. Repeat more than once. If a controlled real condition can make the local/send-level `PubSubClient::subscribe()` call return false, capture that separately. Broker ACL is a deployment prerequisite, not a required firmware-detection test: PubSubClient 2.8 does not expose broker SUBACK grant/rejection. | Interruption produces retained LWT `OFFLINE`, bounded retry, then retained `ONLINE`; any queued door edges arrive FIFO before retained full state and command callbacks resume. A local/send-level `subscribe()` failure produces no false ready state, sets device MQTT state false, disconnects, and retries with the same bounded backoff. | timestamped serial/broker capture; local/send-failure setup if actually available |
...
- P1-M01/P1-M02: SG90 was observed moving at latch-lock `80°` and
  latch-unlock `170°`; no
...
- P1-M03/P1-M07–P1-M11: external power, combined load, captive portal,
  ...
  power/full-load is accepted only as the documented demo risk.
...
| P2-A09/A10 | Node security timeline | one UNLOCK ACK grants one OPEN; later OPEN without a grant (including a legacy producer still `UNLOCKED`); same-state UNLOCK re-arm; expiry/restart; firmware `authorized` override; state reconciliation and episode dedupe | deterministic firmware-first classification; later ungranted OPEN is unauthorized; one bounded `ALARM_ON`/episode |
...
| Browser | `npm run test:dashboard-browser` | local Chrome/CDP auth, action gates, history/chart, keyboard, reduced motion and exact-width reflow without external services | PASS — 18 checks at 1280×720 and 320×800 |
...
## 2026-08-18 browser/audit addendum

Chrome/CDP checks with safe local mocked routes were rerun at desktop 1280×720
and an exact 320×800 window with a classic vertical scrollbar. The 18-check
harness covers auth privacy, action-level gates, 5-second visible polling,
history/chart/table, focus, targets, reduced motion and reflow. Matching async,
stale, failure and single-flight behavior is covered by the 177-test Node suite.
`playwright-cli` on Node `v24.14.1` is separately `UNAVAILABLE`
...
| UI-A03 target/status/motion | PASS | 24 visible effective interaction targets after login; none below 24×24 CSS px; reduced motion applied. |
...
| UI-F03 pending spinner | PASS — fixed | Centered 17 px spinner and `aria-busy` appear only on the requested action; both same-domain buttons remain disabled to prevent a conflicting command. |
...
| UI-F08 mechanical wording | PASS — fixed | UI says **Khóa ngay/Mở chốt**, explains manual door movement plus local auto-lock, and never presents ACK as angle feedback. |
| UI-F09 exact 320 px reflow | PASS — fixed | Removed the `min-width:320px`/classic-scrollbar overflow; layout viewport and document width both measure 305 CSS px inside the 320 px Chrome window. |
...
MQTT example, Wokwi safe-boot/help and latch-versus-door terminology. QoS0 ambiguity
