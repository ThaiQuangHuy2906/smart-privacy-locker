...
The SG90 is not attached in `setup()`; no NVS lock position is restored and no command is replayed. After a broker reconnect the client publishes retained `ONLINE`, full state, and only then subscribes to command. A Node-RED consumer must still use availability and staleness, not retained state alone, to decide whether the device is live.
