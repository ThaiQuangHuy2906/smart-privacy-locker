...
The SG90 is not attached in `setup()`; no NVS lock position is restored and no command is replayed. After a broker reconnect the client sends the command `SUBSCRIBE` packet. PubSubClient 2.8 reports only local/send success for that packet, not broker `SUBACK` confirmation or grant. The firmware uses that local boundary to mark MQTT connected and publish retained `ONLINE` followed by full state; callbacks are processed later from `mqtt_.loop()`. A local/send failure instead leaves MQTT state false, publishes retained `OFFLINE` when possible, disconnects, and follows the bounded retry schedule. A Node-RED consumer must still use availability and staleness, not retained state alone, to decide whether the device is live.
