...
(`functionExternalModules`). This export requests only the Node.js built-ins
`crypto` and `https`; it does not install third-party runtime code. Add the
...
FlowFuse. In particular, Phase 2 neither needs nor permits
`SUPABASE_SERVICE_ROLE_KEY` in Node-RED/Dashboard. `MQTT_USERNAME` and
...
3. Set only the runtime variables listed in the FlowFuse section above in the deployment secret/environment store. Self-hosted Node-RED additionally needs `NODE_RED_CREDENTIAL_SECRET`; Phase 2 does not need `SUPABASE_SERVICE_ROLE_KEY`. Never import a plaintext credential flow.
